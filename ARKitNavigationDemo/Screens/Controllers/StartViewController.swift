...
final class StartViewController: UIViewController, Controller {
    ...
    @IBOutlet weak var SelectorView: UISegmentedControl!
    @IBOutlet weak var SelectorNavigationType: UISegmentedControl!
    ...
    var navigationType: MKDirectionsTransportType = .walking
    ...
    @IBAction func BtnSeacrch_TouchUpInside(_ sender: Any)
    {
        ActivityLoading.isHidden = false
        ActivityLoading.startAnimating()
        if txf_Search.text != ""
        {
            coordinates(forAddress: txf_Search.text!) {
                (location) in
    ...
    @IBAction func ViewSelectorChanged(_ sender: Any)
    {
        switch  SelectorView.selectedSegmentIndex {
        case 0:
            SearchVIew.isHidden = true
        case 1:
    ...
    @IBAction func NavigationTypeSelectorChanged(_ sender: Any)
    {
        switch  SelectorNavigationType.selectedSegmentIndex {
        case 0:
            navigationType = .walking
        case 1:
            navigationType = .automobile
        default:
            return
        }
    }
    ...
    private func setupNavigation() {
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            if self.destinationLocation != nil {
                self.navigationService.getDirections(destinationLocation: self.destinationLocation, transportType: self.navigationType , request: MKDirectionsRequest()) { steps in
                    for step in steps {
    ...
    private func addMapAnnotations() {
        let lastcoordinate = annotations.last?.coordinate;
        let lastAnotation = POIAnnotation(coordinate: lastcoordinate!, name: "Last")
        annotations.append(lastAnotation)
